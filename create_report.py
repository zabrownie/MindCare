...
def create_mindcare_report():
    # Create a new Document
    doc = Document()
    
    # Add title
    title = doc.add_heading('MindCare: Mind Journal Website - Project Report', 0)
...
The project includes comprehensive automated testing with Jest and Supertest, achieving 80% test coverage across all critical functionality. This ensures code reliability, facilitates future maintenance, and prevents regression bugs during development iterations.'''
        ...
        {
            'title': '4. Frontend and Backend Technologies Used',
            'content': '''MindCare leverages a modern, full-stack technology stack designed for performance, scalability, and developer productivity. The technology choices reflect current industry best practices and ensure the platform can evolve with user needs and technological advancements.
...
Joi provides comprehensive schema validation, ensuring data integrity and security. JWT (JSON Web Tokens) handles secure authentication and session management. Jest and Supertest enable comprehensive testing of both unit and integration scenarios, ensuring code quality and reliability throughout the development lifecycle.'''
        ...
        {
            'title': '5. Snapshot of the Project',
            'content': '''MindCare presents a visually appealing and professionally designed user interface that prioritizes user experience and accessibility. The platform's design philosophy centers around creating a calming, distraction-free environment that encourages self-reflection and emotional expression.
...
On larger screens, the interface expands to utilize available space effectively while maintaining visual balance and readability. The layout adjusts dynamically, with navigation elements repositioning and content areas expanding to provide optimal viewing and interaction experiences.'''
        ...
        {
            'title': '7. Database Used: Description, Merits, and Demerits',
...
The journals table maintains a one-to-many relationship with users, allowing each user to create multiple journal entries while maintaining referential integrity through foreign key constraints. The journal entries include metadata such as creation timestamps, mood indicators, and content categorization, enabling future analytics and reporting features.'''
        ...
        {
            'title': '9. Discussion of Project Issues and Limitations',
            'content': '''Development Challenges and Solutions:
...
MindCare represents not only a functional mental health tool but also a showcase of modern web development practices, highlighting the importance of security, testing, and user experience in creating impactful digital solutions for mental health support and personal growth.'''
    ...
    doc.add_paragraph('Sample Code Snippets and Technical Implementation Details')
    doc.add_paragraph()
    
    # Add code examples
    doc.add_heading('User Registration Controller (Node.js)', level=2)
    doc.add_paragraph('```javascript\nconst register = async (req, res) => {\n  // Joi validation and user creation logic\n  const { error, value } = registerSchema.validate(req.body);\n  if (error) {\n    return res.status(400).json({\n      success: false,\n      message: "Validation error",\n      error: error.details[0].message\n    });\n  }\n  // ... rest of implementation\n};\n```')
    
    doc.add_heading('Jest Test Example', level=2)
    doc.add_paragraph('```javascript\nit("should register a new user successfully", async () => {\n  const res = await request(app)\n    .post("/auth/register")\n    .send({\n      name: "Test User",\n      email: "test@example.com",\n      password: "securepassword123"\n    });\n  expect(res.statusCode).toBe(201);\n  expect(res.body.success).toBe(true);\n});\n```')
    
    doc.add_heading('Frontend Auth Context (React)', level=2)
    doc.add_paragraph('```jsx\nimport { createContext, useContext, useState } from "react";\n\nexport const AuthContext = createContext();\n\nexport const AuthProvider = ({ children }) => {\n  const [user, setUser] = useState(null);\n  const [token, setToken] = useState(localStorage.getItem("token"));\n  \n  // Authentication logic implementation\n  \n  return (\n    <AuthContext.Provider value={{ user, token, login, logout }}>\n      {children}\n    </AuthContext.Provider>\n  );\n};\n```')
    
    # Save the document
    doc.save('MindCare_Project_Report.docx')
    print("Report generated successfully as 'MindCare_Project_Report.docx'")

if __name__ == "__main__":
    create_mindcare_report() 